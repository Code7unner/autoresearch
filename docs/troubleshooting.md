...
**Symptom:** `autoresearch doctor` shows Xueqiu as ⚠️ with an `HTTP Error 400`.
...
Run `autoresearch doctor` again to confirm it returns to ✅. Once the cookie expires, just run the command again.
...
>
> **Fallback:** If you already have the bird CLI installed (`npm install -g @steipete/bird`), it works as well. autoresearch automatically detects installed tools.
