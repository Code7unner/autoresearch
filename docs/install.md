...
```
Help me install autoresearch: https://raw.githubusercontent.com/Code7unner/autoresearch/main/docs/install.md
```

> 🛡️ **Security-conscious?** Use safe mode — it won't auto-install system packages:
> ```
> Help me install autoresearch (safe mode): https://raw.githubusercontent.com/Code7unner/autoresearch/main/docs/install.md
...
Install autoresearch and all upstream tools so your user's agent has full internet access. After installation, you'll use the upstream tools directly (twitter-cli, rdt-cli, xhs-cli, yt-dlp, mcporter, gh CLI, etc.) — autoresearch is just the installer and health checker.
...
- **DO NOT** modify system files outside `~/.autoresearch/`
...
All autoresearch files go in dedicated directories — **never in the agent workspace**:

| Purpose | Directory | Example |
|---------|-----------|---------|
| Config & tokens | `~/.autoresearch/` | `~/.autoresearch/config.json` |
| Upstream tool repos | `~/.autoresearch/tools/` | `~/.autoresearch/tools/douyin-mcp-server/` |
| Temporary files | `/tmp/` | `/tmp/yt-dlp-output/` |
| Skills | `~/.openclaw/skills/autoresearch/` | SKILL.md |
...
python3 -m venv ~/.autoresearch-venv
...
> This is PEP 668 protection, not a problem with autoresearch itself. Prefer `pipx install ...`, or create a `venv` first and then install.

**Safe mode / Dry run:**

```bash
autoresearch install --env=auto --safe      # Check only, no auto-install
...
```bash
autoresearch install --env=auto --channels=twitter,weibo   # Example: user chose Twitter + Weibo
autoresearch install --env=auto --channels=all              # User wants everything
```
...
Run `autoresearch doctor` and check the output.
...
>
> **Local-computer users** can also use `autoresearch configure --from-browser chrome` to auto-extract everything in one step (supports Twitter + XiaoHongShu + Xueqiu).
...
> 1. Confirm the user has configured a proxy: `autoresearch configure proxy http://user:pass@ip:port`
> 2. Set the environment variables: `export HTTP_PROXY="..." HTTPS_PROXY="..."`
> 3. autoresearch handles the rest automatically — no extra steps required from the user
>
...
```bash
autoresearch configure proxy http://user:pass@ip:port
```
...
> autoresearch configure xhs-cookies '[{"name":"web_session","value":"xxx","domain":".xiaohongshu.com",...}]'
>
> # Or Header String format (Cookie-Editor → Export → Header String)
> autoresearch configure xhs-cookies "key1=val1; key2=val2; ..."
> ```
>
...
```bash
autoresearch configure --from-browser chrome
```
...
The script is installed automatically with autoresearch; the user only needs to provide a key:

```bash
autoresearch configure groq-key gsk_xxxxx
```
...
> ```bash
> bash ~/.autoresearch/tools/xiaoyuzhou/transcribe.sh https://www.xiaoyuzhoufm.com/episode/xxxxx
> ```
>
...
mkdir -p ~/.autoresearch/tools && cd ~/.autoresearch/tools
...
Run `autoresearch doctor` one final time and report the results to your user.

### Step 5: Set up daily monitoring (OpenClaw only)

If you are running inside **OpenClaw**, ask your user:

> "autoresearch is installed. Want me to set up a task that checks it automatically every day? It will keep an eye on whether these channels are working and whether there are new versions. It only notifies you when there's a problem — otherwise it stays quiet."

If the user agrees, create a **cron job** (daily, `sessionTarget: "isolated"`, `delivery: "announce"`) with this task:

```
Run the autoresearch watch command.
...
|---------|-------------|
| `autoresearch install --env=auto` | Install core channels (lightweight, zero-config) |
| `autoresearch install --env=auto --channels=twitter,weibo` | Install core + optional channels |
| `autoresearch install --env=auto --channels=all` | Install everything |
...
| `autoresearch configure proxy URL` | Unlock Reddit + Bilibili on servers |
| `autoresearch configure groq-key gsk_xxx` | Unlock Xiaoyuzhou podcast transcription |
