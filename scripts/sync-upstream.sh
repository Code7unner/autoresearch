...
LOCAL_DIR="autoresearch/channels"

echo "👁️ autoresearch — Upstream Sync"
...
for upstream_file in "$TMPDIR/upstream/$UPSTREAM_DIR"/*.py; do
    ...
    if ! diff -q <(sed 's/x_reader\.fetchers/autoresearch.channels/g' "$upstream_file") "$local_file" > /dev/null 2>&1; then
        echo "📝 CHANGED: $filename"
        diff --color -u <(sed 's/x_reader\.fetchers/autoresearch.channels/g' "$upstream_file") "$local_file" | head -20
        echo "   ..."
        echo ""
        CHANGES=$((CHANGES + 1))
    fi
...
if [ $CHANGES -eq 0 ]; then
    ...
    echo "  sed -i 's/x_reader\\.fetchers/autoresearch.channels/g' $LOCAL_DIR/FILENAME.py"
    echo ""
