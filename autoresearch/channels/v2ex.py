...
class V2EXChannel(Channel):
    ...
    def check(self, config=None):
        try:
            ...
            return "warn", f"V2EX API connection failed (proxy may be required): {e}"
    ...
    def get_topic(self, topic_id: int) -> dict:
        """Get details of a single topic and its list of replies.
    ...
    def search(self, query: str, limit: int = 10) -> list:
        ...
        proxying the V2EX on-site search page through Jina Reader (plain text, no
        structured data).

        For precise search, consider visiting https://www.v2ex.com/?q=<query> directly
        or using site:v2ex.com search via the Exa channel.

        Returns:
            list of dicts with keys: title, url, snippet
            If search is unavailable, returns a list containing a single {"error": str}.
        """
        return [
            {
                "error": (
                    "The V2EX public API does not provide a search endpoint. "
                    f"Consider using: https://www.v2ex.com/?q={query} "
                    "or use site:v2ex.com search via the Exa channel."
                )
            }
        ]
