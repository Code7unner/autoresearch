# -*- coding: utf-8 -*-
"""
autoresearch CLI — installer, doctor, and configuration tool.
...
def main():
    ...
    parser.add_argument("--version", action="version", version=f"autoresearch v{__version__}")
    ...
    p_uninstall.add_argument("--keep-config", action="store_true",
                             help="Remove skill files only, keep ~/.autoresearch/ config and tokens")
    ...
    if args.command == "version":
        print(f"autoresearch v{__version__}")
        sys.exit(0)
...
def _cmd_install(args):
    ...
    config = Config()
    print()
    print("autoresearch Installer")
    print("=" * 40)

    # Ensure tools directory exists (for upstream tool repos)
    tools_dir = os.path.expanduser("~/.autoresearch/tools")
    ...
    if env == "local" and needs_cookies and not safe_mode and not dry_run:
        ...
        try:
            from autoresearch.cookie_extract import configure_from_browser
    ...
    if env == "server":
        ...
        print("   For Bilibili full access: autoresearch configure proxy http://user:pass@ip:port")
    ...
    if not dry_run:
        ...
        if not requested_channels:
            ...
            print("   autoresearch install --channels=twitter,weibo,xiaohongshu,...")
            print("   autoresearch install --channels=all  (install everything)")

        # Star reminder
        print()
        print("If autoresearch helped you, please give it a Star so more people can find it:")
...
def _install_skill():
    """Install autoresearch as an agent skill (OpenClaw / Claude Code / .agents)."""
    import os
    import shutil
    ...
    for skill_dir in skill_dirs:
        if os.path.isdir(skill_dir):
            target = os.path.join(skill_dir, "autoresearch")
    ...
    if not installed:
        ...
        target = os.path.expanduser("~/.agents/skills/autoresearch")
...
def _uninstall_skill():
    ...
    skill_dirs = [
        ("~/.openclaw/skills/autoresearch", "OpenClaw"),
        ("~/.claude/skills/autoresearch", "Claude Code"),
        ("~/.agents/skills/autoresearch", "Agent"),
    ]
    ...
    if openclaw_home:
        skill_dirs.insert(
            0,
            (os.path.join(openclaw_home, ".openclaw", "skills", "autoresearch"), "OpenClaw"),
        )
...
def _cmd_format(args):
    ...
    if args.platform == "xhs":
        from autoresearch.channels.xiaohongshu import format_xhs_result as formatter
    elif args.platform == "hn":
        from autoresearch.channels.hackernews import format_hn_result as formatter
    else:
        return
...
def _install_xiaoyuzhou_deps():
    ...
    tools_dir = os.path.expanduser("~/.autoresearch/tools/xiaoyuzhou")
    ...
    if has_key:
        ...
        print("     Then run: autoresearch configure groq-key gsk_xxxxx")
...
def _cmd_configure(args):
    ...
    if args.from_browser:
        ...
        print()
        if found_any:
            print("✅ Cookies configured! Run `autoresearch doctor` to see updated status.")
        else:
    ...
    # ── Manual configure ──
    if not args.key:
        print("Usage: autoresearch configure <key> <value>")
    ...
    if args.key == "proxy":
        ...
        print(f"✅ Proxy configured for Bilibili!")
        print("  Note: Reddit now uses rdt-cli for access, no proxy needed.")
        ...
        if auth_token and ct0:
            ...
            print("   1. autoresearch configure twitter-cookies AUTH_TOKEN CT0")
            print('   2. autoresearch configure twitter-cookies "auth_token=xxx; ct0=yyy; ..."')
...
def _configure_xhs_cookies(value):
    ...
    if not value:
        ...
        print("   Usage: autoresearch configure xhs-cookies '<cookie JSON or header string>'")
        return
    ...
    if not docker:
        ...
        cookie_path = os.path.expanduser("~/.autoresearch/xhs-cookies.json")
    ...
    if mcporter:
        ...
        try:
            ...
            if "logged" in result.stdout.lower():
...
def _cmd_uninstall(args):
    """Remove all autoresearch config, tokens, and skill files."""
    import shutil
    import subprocess
    ...
    print()
    print("autoresearch Uninstaller")
    print("=" * 40)
    ...
    # ── 1. Config directory (~/.autoresearch/) ──
    ...
    if dry_run:
        ...
        if removed_any:
            print("autoresearch data removed.")
        else:
            print("Nothing to remove — already clean.")

    print()
    print("Optional: remove the autoresearch Python package itself:")
    print("  pip uninstall autoresearch")
    print()
    print("Optional: remove tools installed by autoresearch:")
...
def _cmd_setup():
    ...
    # Step 2: GitHub token
    print("[Optional] GitHub Token — raise API rate limit")
    ...
    print("[Info] Reddit — search and read via rdt-cli, no configuration needed")
    print("  Install: pipx install rdt-cli")
    print()

    # Step 4: Groq (Whisper)
    print("[Optional] Groq API — speech-to-text when a video has no captions")
    ...
    print("Run autoresearch doctor to see the full status")
    print()
...
def _cmd_watch():
    ...
    if not issues and not update_available:
        print(f"autoresearch: all good ({ok}/{total} channels available, v{__version__} is up to date)")
        return

    print(f"autoresearch monitoring report")
    print(f"=" * 40)
